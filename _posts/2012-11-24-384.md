...
在我较早的一篇文章[《利用Dictionary的弱引用特性实现动态内存管理》](http://www.idom.me/articles/223.html)中详细介绍了使用弱引用的好处。下面贴两个工具类代码，充分利用了弱引用的特性，实现动态内存管理，既可以重用或共享对象，又不需要担心内存回收问题。
